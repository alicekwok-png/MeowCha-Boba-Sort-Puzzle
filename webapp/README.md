...
- 色數階梯（`levels.js colorsFor`）：L1 2 色 → L4 4 → L7 5 → L11 6 → L13 7 → L17 8 → L23 9 色。L1–12 色組照夜市 brief A4 分配表。
- **L27 起一色兩樽**（`bottlesPerColorFor`，用戶 2026-09-06 對齊參考遊戲規模）：一隻色要裝滿兩樽先做完，同樣色數可以鋪多一倍樽。真樽 = 色 × 每色樽數 + 2，盤面由 12 隻升到 21 隻（L27–30 六色 15 隻 · L31–34 七色 17–18 · L35–37 八色 19–20 · L38–40 九色 21）。樽高由 fitScale 統一縮：12 隻 0.190、15 隻 0.168、17–19 隻 0.147、21 隻 0.142。
- ⚠ **大盤面輸唔到**（實測）：14 隻樽嘅致命錯步率只得 ~2%，8 隻樽嗰陣係 20–50% —— 樽越多，同色倒去嘅目標越多，逃生路越闊。所以 L27+ 唔再要求致命錯步（`minFatalRate` null），難度改為靠解法長度（最優 33 → 54 步）、步數上限同隱藏密度（64–65%）。L1–26 維持細盤面同致命錯步門檻。
