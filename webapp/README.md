...
7. **版面**（實作指令 v4 §3）：CSS grid 三行 **20% 頂區（HUD 5.5% + 委託人 10% + 訂單槽 4.5%，HUD 玻璃層唔壓角色）· 70% 盤面 · 10% 道具列（撤銷 / 提示 / +樽 / 重來）**（用戶 2026-09-05：由 UI 攞返垂直空間畀 3 行樽，唔縮樽；委託人必須拎足 10%）；貓助手唔再佔行，只喺交貨（`CAT.showDurationMs` 1.2 s）同教學句（≈2.5 s）由盤面右下滑入再淡出（`#cat-pop`，pointer-events none）。`src/core/layout.js safeLayout()` 以關卡號做種子（練習關用 seed hash），R2 永不遮液體；`?jitter=0` 強制純網格除錯。
