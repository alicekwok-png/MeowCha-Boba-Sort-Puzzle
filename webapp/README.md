...
7. **版面**（實作指令 v4 §3）：CSS grid 三行 **23.5% 頂區（HUD 5.5% + 委託人 13.5% + 訂單槽 4.5%，HUD 玻璃層唔壓角色）· 69.5% 盤面 · 7% 道具列（撤銷 / 提示 / +樽 / 重來）**（用戶 2026-09-06：道具列縮到最細，空間畀委託人；委託人實際高度受槽闊限制，角色圖准闊到 118% 槽闊）；貓助手唔再佔行，只喺交貨（`CAT.showDurationMs` 1.2 s）同教學句（≈2.5 s）由盤面右下滑入再淡出（`#cat-pop`，pointer-events none）。`src/core/layout.js safeLayout()` 以關卡號做種子（練習關用 seed hash），R2 永不遮液體；`?jitter=0` 強制純網格除錯。
...
- L1 教學（純倒、2 隻空瓶）；L2 起 `?` 隱藏層樽 + 廣告樽（L2 有 2 隻）；L1 就有 1 個委託槽、L3 第二個；L4 起只有 1 隻空瓶；L5 Undo；L11 道具列「+樽」+ 廣告委託槽；L12 步數上限；L14 提示；L19 布遮瓶；L21 起廣告委託槽 2 個（免費 2 + 廣告 2 = 4 位委託人全開，到底）。第三免費委託槽（原 L36）已取消。曲頸瓶 / 裂瓶已喺 v4 移除。
- **真樽數係唯一有效嘅難度槓桿**（2026-09-05 用 `npm run scan` 實證：加段數冇用，最優變長預算跟住長）：L1–3 教學 5–6 樽 · L4–6 8 · L7–12 9 · L13–20 10 · L21+ 11；廣告樽受螢幕上限 12 隻夾住（`levels.js cupsFor / adBottlesFor / MAX_BOTTLES_ON_SCREEN`）；空樽維持 1。
- 目標：L4 起亂撳★2 率（隨機玩家喺最優 × 1.5 步內過關）< 10% — `randomTwoStarMax 0.10` 由 generator 用 seed 篩選（估計值要低過門檻 2.5%），`npm run scan` 覆核。
