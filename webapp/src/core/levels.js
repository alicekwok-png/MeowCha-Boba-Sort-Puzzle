...
export function segmentsFor(id, colors, bpc = 1) {
  ...
  if (id >= REVERSE_FROM) {
    // L5–40：0.80 → 0.92；L41–80：0.92 → 1.00（1.00 = 每一格都自成一段，盤面碎到極限）
    const ratio = id <= 40
      ? 0.80 + (id - REVERSE_FROM) * (0.12 / 35)
      : Math.min(1, 0.92 + (id - 40) * (0.08 / 40));
...
/**
 * 由邊一關開始收到 1 空樽。設成 999 = 全部關都係 2 隻（用戶 2026-09-06 拍板）。唔好用「最後一關 + 1」做值 ——
 * 加關嗰陣會靜靜咁令新關變成 1 空樽（2026-09-06 加 L41–80 時就中過一次）。
 ...
 */
export const TIGHT_EMPTY_FROM = 999;
...
export function colorsFor(id) {
  ...
  if (id <= 27) return 8;
  if (id <= 32) return 9;
  if (id <= 65) return MAX_COLORS_PER_LEVEL;
  // L66 起一色三樽：7 × 3 + 2 空樽 = 23 隻，加 1 隻廣告樽啱啱 24（螢幕上限）
  return 7;
}
...
export function bottlesPerColorFor(id) {
  if (id >= 66) return 3;    // 第 14–16 章：一色三樽（84 格，全遊戲最大盤面）
...
export const CAMPAIGN = [
  ...
  R(39, 2, 1, 2, '最後衝刺'),
  R(40, 2, 1, 2, '實驗室之光'),
  // ---- 用戶 2026-09-06「40 關好快玩完」：L41–80 沿用同一套規格。L33–40 已經頂到
  //      24 樽 / 10 色 / 72% 隱藏，所以呢 40 關推嘅係仲郁得到嘅三樣：
  //      段數碎裂度 0.92 → 1.00、布遮樽 3 → 4 → 5、步數上限 +8 → +7 → +6 → +5，
  //      再加 L66 起一色三樽（84 格，全遊戲最大盤面）。----
  // ---- 第 0章：首席藥師 ----
  R(41, 2, 0, 2, '交接班'),
  R(42, 2, 0, 2, '首席的檯'),
  R(43, 2, 0, 2, '十色齊備'),
...
export const CHAPTERS = [
  ...
  { from: 41, to: 45, name: '首席藥師' },
  { from: 46, to: 50, name: '密室' },
  { from: 51, to: 55, name: '古方' },
  { from: 56, to: 60, name: '月蝕' },
  { from: 61, to: 65, name: '霜降' },
  { from: 66, to: 70, name: '長夜' },
  { from: 71, to: 75, name: '賢者之石' },
  { from: 76, to: 80, name: '破曉' },
];
